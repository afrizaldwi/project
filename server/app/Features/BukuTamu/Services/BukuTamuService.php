...
class BukuTamuService
{
    public function __construct(
        private readonly BukuTamuRepositoryInterface $repository
    ) {}

    public function getPaginatedForAdmin(Request $request): LengthAwarePaginator
    {
        ...
        return $paginator->through(fn(BukuTamu $item) => $this->formatTamu($item));
    }
    ...
    {
        $query = $this->buildBaseQuery()
            ->where('bertemu_dengan', $userId)
            ->orderByDesc('waktu_berkunjung')
            ->get();

        return $query->map(fn(BukuTamu $item) => $this->formatTamu($item))->toArray();
    }

    public function getPenghuniAktif(): array
    {
        return RiwayatSewa::with(['user', 'kamar'])
    ...
    public function create(array $data, int $userId): BukuTamu
    {
        return $this->repository->create([
            'nama_tamu' => $data['nama_tamu'],
            'no_hp_tamu' => $data['no_hp_tamu'],
            'bertemu_dengan' => $userId,
    ...
    {
        return BukuTamu::with('dikunjungi');
    }
    ...
    public function formatTamu(BukuTamu $item): array
    {
